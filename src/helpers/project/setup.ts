...
// setup package .xps database folders with a prefix
export async function initDatabase(setupDir: string = process.cwd(), prefix = 'temp') {
  const projectDir = path.resolve(setupDir, Constants.XPS_PROJECT_DIR, prefix)
